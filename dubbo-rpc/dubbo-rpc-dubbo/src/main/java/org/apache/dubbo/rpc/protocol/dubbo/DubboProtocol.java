...
public class DubboProtocol extends AbstractProtocol {
    ...
    private ExchangeHandler requestHandler = new ExchangeHandlerAdapter() {
        ...
        public CompletableFuture<Object> reply(ExchangeChannel channel, Object message) throws RemotingException {
            if (message instanceof Invocation) {
                ...
                Invoker<?> invoker = getInvoker(channel, inv);// Exporter -> Invoker
    ...
    private void optimizeSerialization(URL url) throws RpcException {
        ...
        try {
            ...
            SerializationOptimizer optimizer = (SerializationOptimizer) clazz.newInstance();//构造URL中optimizer指定的class
    ...
    private ExchangeClient initClient(URL url) {
        ...
        url = url.addParameter(Constants.CODEC_KEY, DubboCodec.NAME);//默认指定dubbo协议
        // enable heartbeat by default
        url = url.addParameterIfAbsent(Constants.HEARTBEAT_KEY, String.valueOf(Constants.DEFAULT_HEARTBEAT));

        // BIO is not allowed since it has severe performance issue.
        if (str != null && str.length() > 0 && !ExtensionLoader.getExtensionLoader(Transporter.class).hasExtension(str)) {//传输层协议不一致
            throw new RpcException("Unsupported client type: " + str + "," +
        ...
        try {
            ...
            if (url.getParameter(Constants.LAZY_CONNECT_KEY, false)) {
                client = new LazyConnectExchangeClient(url, requestHandler);//使用时再连接
            } else {
