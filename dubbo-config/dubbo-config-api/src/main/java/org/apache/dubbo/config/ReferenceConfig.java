...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
    ...
    private void init() {
        ...
        checkDefault();
        appendProperties(this);//将系统中各个值赋值给ReferenceConfig
        ...
        checkApplication();
        checkStubAndMock(interfaceClass);//local,stub,mock
        ...
        if (!isGeneric()) {
            ...
            if (methods.length == 0) {
                ...
                map.put("methods", StringUtils.join(new HashSet<String>(Arrays.asList(methods)), ","));//interface的所有方法名用,拼接
            }
        }
        map.put(Constants.INTERFACE_KEY, interfaceName);
        appendParameters(map, application);//将application中的get方法代表的属性的值，都转到map中
        appendParameters(map, module);
        appendParameters(map, consumer, Constants.DEFAULT_KEY);
        appendParameters(map, this);
        String prefix = StringUtils.getServiceKey(map);//返回格式如下：group/interface:version
        if (methods != null && !methods.isEmpty()) {
            for (MethodConfig method : methods) {
                ...
                appendAttributes(attributes, method, prefix + "." + method.getName());//从methodConfig中将get方法对应的属性的值，塞入attribute
    ...
    private T createProxy(Map<String, String> map) {
        ...
        if (isInjvm() == null) {//<dubbo:reference/>属性
        ...
        if (isJvmRefer) {
            ...
            if (url != null && url.length() > 0) { // user specified URL, could be peer-to-peer address, or register center's address.
                String[] us = Constants.SEMICOLON_SPLIT_PATTERN.split(url);//;作为分隔符
                if (us != null && us.length > 0) {//主要做的事情就是将map中的键值对拼接到url上，组成一个新的url
                    for (String u : us) {
                        URL url = URL.valueOf(u);
                        if (url.getPath() == null || url.getPath().length() == 0) {
                            url = url.setPath(interfaceName);//默认的path就是接口的全限定名
                        }
                ...
                List<URL> us = loadRegistries(false);//将ApplicationConfig,RegistryConfig中的属性值以及一些额外的属性[protocol,pid...]组装成一URL对象
            ...
            if (urls.size() == 1) {
                ...
                for (URL url : urls) {
                    invokers.add(refprotocol.refer(interfaceClass, url));//interfaceClass是指xml配置中的interface属性指定的接口
                    if (Constants.REGISTRY_PROTOCOL.equals(url.getProtocol())) {
    ...
    public void setInterface(String interfaceName) {//通过Spring提供的接口BeanDefinitionParser[实际上是DubboBeanDefinitionParser]解析xml的时候注入该属性值
