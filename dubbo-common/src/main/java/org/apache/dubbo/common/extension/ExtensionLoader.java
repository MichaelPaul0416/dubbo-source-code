...
public class ExtensionLoader<T> {
    ...
    public static <T> ExtensionLoader<T> getExtensionLoader(Class<T> type) {//获取扩展的类加载器
        if (type == null)
        ...
        if (loader == null) {
            // key是有SPI注解修饰的Class，value是ExtensionLoader实例对象，也就是说，一个SPI注解修饰的接口，对应一个ExtensionLoader实例
        ...
        try {
            ...
            if (wrapperClasses != null && !wrapperClasses.isEmpty()) {
                for (Class<?> wrapperClass : wrapperClasses) {
                    // 如果是wrapper的话，那么获取wrapper的有参构造器，将当前构造好的实例对象，作为参数传给有参构造器
                ...
                for (Method method : instance.getClass().getMethods()) {
                    if (method.getName().startsWith("set")
                        ...
                        try {
                            ...
                            // 获取属性的实例对象，property就是拿来获取spi实例的name
                            // 对于spi获取的方式，其实最后还是通过META-INF下面的spi文件指定的实现类，执行它的构造器来实现实例的构造
                            // 委托给指定的ExtensionFactory来获取bean
    ...
    private Map<String, Class<?>> loadExtensionClasses() {
        // type属于实例对象，就是在构造器中输入的Class<T>
        final SPI defaultAnnotation = type.getAnnotation(SPI.class);//一般来说，加载某个SPI的实现，首先都会来加载ExtensionFactory的实现，然后根据ExtensionFactory获得对应的SPI修饰的接口的具体实现
        if (defaultAnnotation != null) {
            ...
            if ((value = value.trim()).length() > 0) {
                ...
                if (names.length == 1) cachedDefaultName = names[0];//@SPI(value) 这个value作为被@SPI修饰的注解的默认实现类
            }
        }
    ...
    private void loadDirectory(Map<String, Class<?>> extensionClasses, String dir, String type) {
        ...
        try {
            ...
            if (classLoader != null) {
                urls = classLoader.getResources(fileName);//搜索classpath下面的
            } else {
    ...
    private void loadClass(Map<String, Class<?>> extensionClasses, java.net.URL resourceURL, Class<?> clazz, String name) throws NoSuchMethodException {
        ...
        if (clazz.isAnnotationPresent(Adaptive.class)) {//被@Adaptive修饰
            ...
            clazz.getConstructor();// 有public的无参构造器
            if (name == null || name.length() == 0) {
                name = findAnnotationName(clazz);// clazz 是SPI接口实现类的class
                if (name.length() == 0) {
                ...
                // did not find parameter in URL type
                    ...
                    for (int i = 0; i < pts.length; ++i) {
                        ...
                        for (Method m : ms) {
                            ...
                            // 方法的入参必须是get开头，并且是无入参，非静态，公开方法，返回值类型必须是URL
