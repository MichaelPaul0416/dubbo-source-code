...
final class NettyCodecAdapter {
    ...
    private final org.apache.dubbo.remoting.ChannelHandler handler;//NettyClient
    ...
    private class InternalDecoder extends SimpleChannelUpstreamHandler {
        ...
        public void messageReceived(ChannelHandlerContext ctx, MessageEvent event) throws Exception {
            ...
            if (buffer.readable()) {
                if (buffer instanceof DynamicChannelBuffer) {//可以动态扩容的话，那就直接写进去
                    buffer.writeBytes(input.toByteBuffer());
                    message = buffer;
                } else {
                    int size = buffer.readableBytes() + input.readableBytes();//计算第一次的总长度，然后构造一个动态扩容的ByteBuffer
            ...
            NettyChannel channel = NettyChannel.getOrAddChannel(ctx.getChannel(), url, handler);//handler的实际类型是NettyClient
            Object msg;
            int saveReaderIndex;
