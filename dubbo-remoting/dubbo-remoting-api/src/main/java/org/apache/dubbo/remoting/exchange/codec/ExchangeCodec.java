...
public class ExchangeCodec extends TelnetCodec {
    ...
    protected Object getRequestData(long id) {
        DefaultFuture future = DefaultFuture.getFuture(id);//DefaultFuture类对象维护了id与DefaultFuture的实例
        if (future == null)
            return null;
    ...
    protected void encodeResponse(Channel channel, ChannelBuffer buffer, Response res) throws IOException {
        ...
        try {
            ...
            byte[] header = new byte[HEADER_LENGTH];
            // set magic number.
            Bytes.short2bytes(MAGIC, header);//0xdabb-->55995写入byte的前两位(short是两个字节)
            // set request and serialization flag.
            ...
            buffer.writerIndex(savedWriteIndex + HEADER_LENGTH);//协议头构造，写入缓冲区
            ChannelBufferOutputStream bos = new ChannelBufferOutputStream(buffer);
            ObjectOutput out = serialization.serialize(channel.getUrl(), bos);
